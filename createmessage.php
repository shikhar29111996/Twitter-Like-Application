...
$member_id = $_SESSION['SESS_MEMBER_ID'];
$received = mysqli_query($connection,"SELECT * FROM messages WHERE receiver = '$member_id'")or die(mysqli_error($connection));
...
<div><form name="" method="post">
							...
							<?php 
								$member_id=$_SESSION['SESS_MEMBER_ID'];							
								$post = mysqli_query($connection,"SELECT * FROM friends WHERE friends_with = '$member_id' OR member_id = '$member_id' AND status = 'conf' ")or die(mysqli_error($connection));
							...
							<br />
								...
								mysqli_query($connection,"INSERT INTO messages (receiver, recipient, datetime, content, status)
								VALUES ('$from','$sendto',NOW(),'$message','unread')")or die(mysqli_error($connection));
